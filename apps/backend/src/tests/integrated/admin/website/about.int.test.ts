...
describe("Admin About (integration-style) — /admin/about", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });
  ...
  describe("PUT /admin/about/:id", () => {
    it("updates about successfully", async () => {
      ...
      expect(prisma.about.findUnique).toHaveBeenCalledWith(
        expect.objectContaining({ where: { id }, include: { image: true } })
      );
      expect(prisma.about.update).toHaveBeenCalledTimes(1);
    });
