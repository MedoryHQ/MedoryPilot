...
describe("Admin Hero (integration-style) — /admin/hero", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  });

  describe("GET /admin/hero", () => {
    it("returns hero when found", async () => {
      ...
      const res = await request(app).get("/admin/hero");
  ...
  describe("GET /hero/:id", () => {
    it("returns 404 when not found", async () => {
      ...
      const res = await request(app).get("/admin/hero/invalidUUID");

      expect(res.status).toBe(404);
      expect(res.body).toHaveProperty(
        "error",
        require("@/utils").errorMessages.heroNotFound
      );
    });
    ...
    it("handles DB error", async () => {
      ...
      const res = await request(app).get("/admin/hero/some-id");

      expect(res.status).toBe(500);
    });
  });

  describe("POST /admin/hero", () => {
    it("creates hero successfully", async () => {
      (prisma.hero.create as jest.Mock).mockResolvedValueOnce(mockHero);

      const res = await request(app).post("/admin/hero").send(createPayload);
  ...
  describe("PUT /admin/hero/:id", () => {
    it("updates hero successfully", async () => {
      ...
      const updateArg = (prisma.hero.update as jest.Mock).mock.calls[0][0];
  ...
  describe("DELETE /admin/hero/:id", () => {
    ...
    it("returns 400 for invalid UUID id", async () => {
      const res = await request(app).delete("/admin/hero/invalid-id");
      expect(res.status).toBe(400);
      expect(res.body).toHaveProperty("errors");
    });
  });
